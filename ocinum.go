...
func (num Num) Print(buf []byte) []byte {
	res := buf[:0]
	b := num[0]
	negative := b&(1<<7) == 0
	exp := int16(b & ((1 << 7) - 1))
	if negative {
		...
		for _, b := range num[:length] {
			res = append(res, '0'+(101-b))
		}
	} else {
		exp += 65 + 128
		for _, b := range num {
			res = append(res, '0'+(b-1))
		}
	}
	return res
}
...
// SetString sets the Num to the number in s.
func (num *Num) SetString(s string) error {
}
