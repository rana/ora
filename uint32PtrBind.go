...
func (b *uint32PtrBind) bind(value *uint32, position int, ocistmt *C.OCIStmt) error {
	b.value = value
	r := C.OCIBindByPos2(
...
func (b *uint32PtrBind) setPtr() error {
	if b.isNull > -1 {
		r := C.OCINumberToInt(
...
func (b *uint32PtrBind) close() {
	defer func() {
		recover()
	}()
	b.ocibnd = nil
	b.value = nil
	b.env.uint32PtrBindPool.Put(b)
}
