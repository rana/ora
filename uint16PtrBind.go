...
func (b *uint16PtrBind) bind(value *uint16, position int, ocistmt *C.OCIStmt) error {
	b.value = value
	r := C.OCIBindByPos2(
...
func (b *uint16PtrBind) setPtr() error {
	if b.isNull > -1 {
		r := C.OCINumberToInt(
...
func (b *uint16PtrBind) close() {
	defer func() {
		recover()
	}()
	b.ocibnd = nil
	b.value = nil
	b.env.uint16PtrBindPool.Put(b)
}
