...
func (b *int16SliceBind) bindOra(values []Int16, position int, ocistmt *C.OCIStmt) error {
	...
	return b.bind(int16Values, nullInds, position, ocistmt)
}

func (b *int16SliceBind) bind(values []int16, nullInds []C.sb2, position int, ocistmt *C.OCIStmt) error {
	if nullInds == nil {
		nullInds = make([]C.sb2, len(values))
	}
...
func (b *int16SliceBind) setPtr() error {
	return nil
}

func (b *int16SliceBind) close() {
	defer func() {
		recover()
	}()
	b.ocibnd = nil
	b.env.int16SliceBindPool.Put(b)
}
