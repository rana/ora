...
func (bnd *bndUint32Slice) bindOra(values *[]Uint32, position int, stmt *Stmt, isAssocArray bool) (iterations uint32, err error) {
	L, C := len(*values), cap(*values)
	bnd.values = values
	if cap(bnd.uints) < C {
		bnd.uints = make([]uint32, L, C)
	} else {
...
func (bnd *bndUint32Slice) bind(values []uint32, position int, stmt *Stmt, isAssocArray bool) (iterations uint32, err error) {
	bnd.stmt = stmt
