...
func (bnd *bndUint8Slice) bindOra(values []Uint8, position int, stmt *Stmt) error {
...
func (bnd *bndUint8Slice) bind(values []uint8, nullInds []C.sb2, position int, stmt *Stmt) error {
	bnd.stmt = stmt
	if nullInds == nil {
		nullInds = make([]C.sb2, len(values))
	}
