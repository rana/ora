...
func (bnd *bndUint8Slice) bindOra(values *[]Uint8, position int, stmt *Stmt, isAssocArray bool) (iterations uint32, err error) {
	L, C := len(*values), cap(*values)
	if cap(bnd.uints) < C {
		bnd.uints = make([]uint8, L, C)
	} else {
	...
	for n := range *values {
		if (*values)[n].IsNull {
			...
			bnd.uints[n] = (*values)[n].Value
		}
	}
	return bnd.bind(bnd.uints, position, stmt, isAssocArray)
}

func (bnd *bndUint8Slice) bind(values []uint8, position int, stmt *Stmt, isAssocArray bool) (iterations uint32, err error) {
	bnd.stmt = stmt
...
func (bnd *bndUint8Slice) setPtr() error {
	if !bnd.IsAssocArr() {
		return nil
	}
	n := int(bnd.curlen)
...
func (bnd *bndUint8Slice) close() (err error) {
	defer func() {
