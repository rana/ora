...
func (bnd *bndUint16Slice) bindOra(values *[]Uint16, position int, stmt *Stmt, isAssocArray bool) (iterations uint32, err error) {
	L, C := len(*values), cap(*values)
	bnd.values = values
	if cap(bnd.uints) < C {
		bnd.uints = make([]uint16, L, C)
	} else {
...
func (bnd *bndUint16Slice) bind(values []uint16, position int, stmt *Stmt, isAssocArray bool) (iterations uint32, err error) {
	bnd.stmt = stmt
...
func (bnd *bndUint16Slice) setPtr() error {
	if !bnd.IsAssocArr() {
		return nil
	}
	n := int(bnd.curlen)
...
func (bnd *bndUint16Slice) close() (err error) {
	defer func() {
