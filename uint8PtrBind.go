...
func (b *uint8PtrBind) bind(value *uint8, position int, ocistmt *C.OCIStmt) error {
	b.value = value
	r := C.OCIBindByPos2(
...
func (b *uint8PtrBind) setPtr() error {
	if b.isNull > -1 {
		r := C.OCINumberToInt(
...
func (b *uint8PtrBind) close() {
	defer func() {
		recover()
	}()
	b.ocibnd = nil
	b.value = nil
	b.env.uint8PtrBindPool.Put(b)
}
