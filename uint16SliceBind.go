...
func (b *uint16SliceBind) bindOra(values []Uint16, position int, ocistmt *C.OCIStmt) error {
...
func (b *uint16SliceBind) bind(values []uint16, nullInds []C.sb2, position int, ocistmt *C.OCIStmt) error {
	if nullInds == nil {
		nullInds = make([]C.sb2, len(values))
	}
...
func (b *uint16SliceBind) setPtr() error {
	return nil
}

func (b *uint16SliceBind) close() {
	defer func() {
		recover()
	}()
	b.ocibnd = nil
	b.env.uint16SliceBindPool.Put(b)
}
