...
stmtCount, err := ses.Prepare("select count(c1) from t1 where c2 is null", U8)
...
stmtProcCreate, err := ses.Prepare(
	...
	"begin open p1 for select c1, c2 from t1 where c1 > 2 order by c1; " +
	"end proc1;")
...
Opening and closing a `ResultSet` is managed internally. `ResultSet` doesn't have an 
...
Further code examples are avilable in the example file and test files.
