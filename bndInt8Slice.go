...
func (bnd *bndInt8Slice) bindOra(values *[]Int8, position int, stmt *Stmt, isAssocArray bool) (iterations uint32, err error) {
	L, C := len(*values), cap(*values)
	if cap(bnd.ints) < C {
		bnd.ints = make([]int8, L, C)
	} else {
		bnd.ints = bnd.ints[:L]
	}
