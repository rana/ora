...
func (b *int8SliceBind) bindOra(values []Int8, position int, ocistmt *C.OCIStmt) error {
	...
	return b.bind(int8Values, nullInds, position, ocistmt)
}

func (b *int8SliceBind) bind(values []int8, nullInds []C.sb2, position int, ocistmt *C.OCIStmt) error {
	if nullInds == nil {
		nullInds = make([]C.sb2, len(values))
	}
...
func (b *int8SliceBind) setPtr() error {
	return nil
}

func (b *int8SliceBind) close() {
	defer func() {
		recover()
	}()
	b.ocibnd = nil
	b.env.int8SliceBindPool.Put(b)
}
