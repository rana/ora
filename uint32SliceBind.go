...
func (b *uint32SliceBind) bindOra(values []Uint32, position int, ocistmt *C.OCIStmt) error {
...
func (b *uint32SliceBind) bind(values []uint32, nullInds []C.sb2, position int, ocistmt *C.OCIStmt) error {
	if nullInds == nil {
		nullInds = make([]C.sb2, len(values))
	}
...
func (b *uint32SliceBind) setPtr() error {
	return nil
}

func (b *uint32SliceBind) close() {
	defer func() {
		recover()
	}()
	b.ocibnd = nil
	b.env.uint32SliceBindPool.Put(b)
}
