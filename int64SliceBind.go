...
func (b *int64SliceBind) bindOra(values []Int64, position int, ocistmt *C.OCIStmt) error {
	...
	return b.bind(int64Values, nullInds, position, ocistmt)
}

func (b *int64SliceBind) bind(values []int64, nullInds []C.sb2, position int, ocistmt *C.OCIStmt) error {
	if nullInds == nil {
		nullInds = make([]C.sb2, len(values))
	}
...
func (b *int64SliceBind) setPtr() error {
	return nil
}

func (b *int64SliceBind) close() {
	defer func() {
		recover()
	}()
	b.ocibnd = nil
	b.env.int64SliceBindPool.Put(b)
}
