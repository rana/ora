...
func (b *float32SliceBind) bindOra(values []Float32, position int, ocistmt *C.OCIStmt) error {
	...
	return b.bind(float32Values, nullInds, position, ocistmt)
}

func (b *float32SliceBind) bind(values []float32, nullInds []C.sb2, position int, ocistmt *C.OCIStmt) error {
	if nullInds == nil {
		nullInds = make([]C.sb2, len(values))
	}
...
func (b *float32SliceBind) setPtr() error {
	return nil
}

func (b *float32SliceBind) close() {
	defer func() {
		recover()
	}()
	b.ocibnd = nil
	b.env.float32SliceBindPool.Put(b)
}
