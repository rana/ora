...
func (b *int32SliceBind) bindOra(values []Int32, position int, ocistmt *C.OCIStmt) error {
	...
	return b.bind(int32Values, nullInds, position, ocistmt)
}

func (b *int32SliceBind) bind(values []int32, nullInds []C.sb2, position int, ocistmt *C.OCIStmt) error {
	if nullInds == nil {
		nullInds = make([]C.sb2, len(values))
	}
...
func (b *int32SliceBind) setPtr() error {
	return nil
}

func (b *int32SliceBind) close() {
	defer func() {
		recover()
	}()
	b.ocibnd = nil
	b.env.int32SliceBindPool.Put(b)
}
